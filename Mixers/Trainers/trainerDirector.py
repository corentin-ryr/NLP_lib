...
class TrainerDirector():
    ...
    def get_binary_trainer(model:nn.Module, device, traindataset:Dataset, testdataset:Dataset, nb_epochs:int=20, batch_size:int=256) -> ClassificationTrainer:
        ...
        return trainer.set_optimizer_and_loss(nn.BCEWithLogitsLoss, optim.Adam).set_validation_metrics({ConfusionMatrix, Accuracy, Recall, Precision}, num_classes=2)
    ...
    @staticmethod
    def get_hmc_trainer(model:nn.Module, device, traindataset:Dataset, testdataset:Dataset) -> ClassificationTrainer:
        trainer = ClassificationTrainerHMC(model, device=device, traindataset=traindataset, testdataset=testdataset)
