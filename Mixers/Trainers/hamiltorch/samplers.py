...
def leapfrog(params, momentum, log_prob_func, steps=10, step_size=0.1, jitter=0.01, normalizing_const=1., softabs_const=1e6, 
             ...
             inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, store_on_GPU = True, debug=False):
...
def rm_hamiltonian(params, momentum, log_prob_func, jitter, normalizing_const, softabs_const=1e6, sampler=Sampler.HMC, 
                   integrator=Integrator.EXPLICIT, metric=Metric.HESSIAN):
...
def hamiltonian(params, momentum, log_prob_func, jitter=0.01, normalizing_const=1., softabs_const=1e6, explicit_binding_const=100, 
                inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.EXPLICIT, metric=Metric.HESSIAN):
...
def sample(log_prob_func, params_init, num_samples=10, num_steps_per_sample=10, step_size=0.1, burn=0, jitter=None, inv_mass=None, 
           normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, 
           jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, desired_accept_rate=0.8, 
           store_on_GPU = True):
            ...
            if rho >= torch.log(torch.rand(1)):
                ...
                if n > burn:
                    if store_on_GPU:
                        ...
                        ret_params.append(leapfrog_params[-1].detach().cpu())
                else:
                ...
                if n > burn:
                    ...
                    if store_on_GPU:
                        ...
                        # Store samples on CPU
                        ret_params.append(ret_params[-1].detach().cpu())
                else:
...
def define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=1., 
    ...
    x : torch.tensor
        Input training data to define the log probability. Should be a shape that can be passed into the model. 
...
def define_split_model_log_prob(model, model_loss, train_loader, num_splits, params_flattened_list, params_shape_list, tau_list, 
                                tau_out, normalizing_const=1., predict=False, device = 'cpu'):
...
def define_full_batch_model_log_prob(model, model_loss, train_loader, params_flattened_list, params_shape_list, tau_list, 
                                    tau_out, normalizing_const=1., predict=False, prior_scale = 1.0, device = 'cpu'):
    """This function defines the list of log_prob_func's to be used for splitting. It follows the same formulation as `define_model_log_prob`, except
    it will split the log_prob_func according to data subsets.
...
def sample_model(model, x, y, params_init, model_loss='multi_class_linear_output' ,num_samples=10, num_steps_per_sample=10, 
...
def sample_split_model(model, train_loader, params_init, num_splits, model_loss='multi_class_linear_output', num_samples=10, 
                       num_steps_per_sample=10, step_size=0.1, burn=0, inv_mass=None, jitter=None, normalizing_const=1., 
                       softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, 
                       jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.SPLITTING, metric=Metric.HESSIAN, debug=False, 
...
def sample_full_batch_model(model, train_loader, params_init, model_loss='multi_class_linear_output' ,num_samples=10, num_steps_per_sample=10, 
    ...
    train_loader: dataloader
        Dataloader of the full dataset on which we want to compute the log probability function.
    params_init : torch.tensor
        Initialisation of the parameters. This is a vector corresponding to the starting point of the sampler: shape: (D,), where D is
        the number of parameters of the model. The device determines which piece of hardware to run on.
    model_loss : {'binary_class_linear_output', 'multi_class_linear_output', 'multi_class_log_softmax_output', 'regression'} or function
...
def predict_model(model, samples, x = None, y = None, test_loader = None, model_loss='multi_class_linear_output', tau_out=1., tau_list=None):
    ...
    with torch.no_grad():
        ...
        if test_loader.__class__ is DataLoader:
            log_prob_list = define_split_model_log_prob(model, model_loss, test_loader, len(test_loader), params_flattened_list, 
                                                        params_shape_list, tau_list, tau_out, normalizing_const=1., predict=True, 
            ...
            log_prob_func = define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, 
                                                  tau_out, predict=True, device = samples[0].device)
...
def inference_model(model, samples, data:torch.Tensor):
    """Function used to make predictions given model samples. Note that either a data loader can be passed in, or two tensors (x,y) but make sure
    not to pass in both.
    ...
    samples : list of torch.tensors
        ...
        The length of the list is given by the number of samples, S.
    data : torch.tensor
        Input data to be evaluated over.
    ...
    """
    with torch.no_grad():
        if data.device != samples[0].device:
            raise RuntimeError('x on device: {} and samples on device: {}'.format(data.device, samples[0].device))

        fmodel = util.make_functional(model)

        pred_list = []
        for s in samples:
            params_unflattened = util.unflatten(model, s)
            pred = fmodel(data, params=params_unflattened)
